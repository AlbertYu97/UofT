...
class GroceryStoreSimulation:
    ...
    def run(self, initial_events: list[Event]) -> None:
        ...
        # add the initial events to self._events, loop over and add to pq
        total_time = 0
        # max_wait time
        customer_max_wait_time = {}
        for event in initial_events:
            self._events.add(event)

        while not self._events.is_empty():
            event = self._events.remove()
            total_time = max(event.timestamp, total_time)
            if isinstance(event, CustomerArrival):
                cx_name = event.customer.name
                # Keep track of customer first arrival time
                # if cx_name not in customer_max_wait_time.keys():
                customer_max_wait_time[cx_name] = event.customer.arrival_time
                # print(cx_name + " in time: " + str(event.customer.arrival_time))
            if isinstance(event, CheckoutCompleted):
                cx_name = event.customer.name
                # print(cx_name + " out time: " + str(event.timestamp))
                # add cx by 1 when cx checkout
                self.stats['num_customers'] += 1
                in_time = customer_max_wait_time[cx_name]
                out_time = event.timestamp
                customer_max_wait_time[cx_name] = out_time - in_time
            # process event and find out what new events this generates
            new_events = event.do(self._store)
            # add any new events to self._events
            if new_events:
                for each_event in new_events:
                    self._events.add(each_event)
            # print(self._events._items[-1])
            # print(event)
        # Update total_time
        self.stats['total_time'] = total_time
        self.stats['max_wait'] = max(customer_max_wait_time.values())
